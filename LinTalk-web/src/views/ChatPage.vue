<template>
  <div class="chat-container">
    ...
    <notify />
    <!--表情弹窗-->
    <lintalk-popup v-model:visible="isEmojiVisible" :position="emojiPosition">
      <lintalk-emoji-box @on-emoji="handlerOnEmoji" />
    </lintalk-popup>
    <div class="chat-bg">
      <div class="chat-box">
        <!-- 左侧菜单 -->
        <div class="box-left" :class="{ 'show-left': showLeft }">
          ...
          <div
            ...
            "
          >
            <lintalk-avatar :info="{ name: '群' }" size="40px" :color="2" class="mr-[10px]" />
            <div class="chat-item-content">
              ...
              <div v-if="groupChat?.lastMessage" class="chat-content-msg">
                <lintalk-chat-list-content :msg="groupChat?.lastMessage" />
              </div>
            </div>
          </div>
          <div
          ...
          <div class="chat-list-content">
            <div
              ...
              "
            >
              <lintalk-avatar :info="item.targetInfo" size="40px" class="mr-[10px]" />
              <div class="chat-item-content">
                <div class="flex items-center mb-[5px]">
                ...
                <div class="chat-content-msg">
                  <lintalk-chat-list-content :is-group="false" :msg="item?.lastMessage" />
                </div>
              </div>
              <lintalk-text-button
        ...
        <div class="box-middle">
          ...
          <div class="middle-content">
            <div class="chat-show-area" ref="chatShowAreaRef">
              <div
                v-for="(item, index) in msgRecord"
                class="msg-item"
                :key="item.id || `${item.createTime || 'msg'}-${index}`"
                :class="{ right: item.fromId === userInfoStore.userId }"
              >
                <lintalk-msg :msg="item" :user="msgStore.userListMap.get(item.fromId)" />
              </div>
            ...
            <div class="chat-input-area">
              <div class="chat-input">
                <div v-if="msgStore.referenceMsg" class="reference-msg">
                  ...
                  >
                    {{ getReferenceSenderName(msgStore.referenceMsg) }} :
                  ...
                  >
                    <div>{{ getReferenceSenderName(msgStore.referenceMsg) }} :</div>
        ...
        <div class="box-right" :class="{ 'show-right': showRight }">
          <div class="right-top">
            ...
            <div class="flex">
              <theme-picker />
              <lintalk-icon-button @click="handlerLogout" icon="icon-tuichu" />
            </div>
          </div>
              ...
              >
                <div class="online-item-content">
                  <div class="w-[40px] h-[40px] relative">
                    <lintalk-avatar :info="item" size="40px" />
...
import LintalkDotHint from '@/components/LintalkDotHint.vue'
...
import LintalkTextButton from '@/components/LintalkTextButton.vue'
import { useToast } from '@/components/ToastProvider.vue'
import LintalkIconButton from '@/components/LintalkIconButton.vue'
import LintalkMsg from '@/components/Msg/LintalkMsg.vue'
import { useChatMsgStore } from '@/stores/useChatMsgStore.js'
import LintalkPopup from '@/components/LintalkPopup.vue'
import LintalkEmojiBox from '@/components/LintalkEmojiBox.vue'
import EmojiMsg from '@/components/Msg/MsgContent/EmojiMsg.vue'
import LintalkChatListContent from '@/components/Msg/LintalkChatListContent.vue'
import { MessageType } from '@/constant/messageType.js'
import { MessageSource } from '@/constant/messageSource.js'
import LintalkMsgInput from '@/components/LintalkMsgInput.vue'
...
import LintalkLoading from '@/components/LintalkLoading.vue'
import ChatSkeleton from '@/components/ChatSkeleton.vue'
import LintalkCardCarousel from '@/components/LintalkCardCarousel.vue'
...
let forceScrollDebounceTimer = null
const isFirstLoadScrolled = ref(false)
...
const mergeUniqueMessages = (messages) => {
  ...
  return [...map.values(), ...noIdMessages]
}
...
const appendMessageUnique = (message) => {
  ...
  const idx = msgRecord.value.findIndex((it) => it?.id === message.id)
  if (idx >= 0) {
    msgRecord.value[idx] = message
  } else {
    msgRecord.value.push(message)
  }
}
...
const getReferenceSenderName = (msg) => {
  if (!msg) return '未知用户'
  return msg.fromInfo?.name || msgStore.userListMap.get(msg.fromId)?.name || '未知用户'
}

const handlerSetEmojiBoxPosition = (e) => {
  const el = e?.currentTarget || e?.target
  if (!el?.getBoundingClientRect) {
    emojiPosition.value = { top: 0, left: 0 }
  } else {
    const rect = el.getBoundingClientRect()
    emojiPosition.value = { top: rect.bottom, left: rect.left }
  }
...
const handlerReceiveMsg = (data) => {
  ...
  if (
    ...
    const existed = data.id && msgRecord.value.some((it) => it?.id === data.id)
...
onUnmounted(() => {
  ...
  if (forceScrollDebounceTimer) {
    clearTimeout(forceScrollDebounceTimer)
    forceScrollDebounceTimer = null
  }
})
...
const scheduleForceScrollToBottom = (delay = 120) => {
  if (forceScrollDebounceTimer) {
    clearTimeout(forceScrollDebounceTimer)
    forceScrollDebounceTimer = null
  }
  forceScrollDebounceTimer = setTimeout(() => {
    scrollToBottom()
    forceScrollDebounceTimer = null
  }, delay)
}
...
const onGetMsgRecord = () => {
  ...
  return MessageApi.record({ targetId: targetId.value, index: recordIndex, num: 20 })
    .then((res) => {
      if (res.code === 0) {
        const newMessages = res.data
        if (newMessages.length > 0) {
          msgRecord.value = mergeUniqueMessages([...newMessages.reverse(), ...msgRecord.value])
          recordIndex = msgRecord.value.length
          nextTick(() => {
...
watch(
  ...
  async (newValue, oldValue) => {
    ...
    await onGetMsgRecord()
    // 每次切换会话后，统一定位到底部显示最新消息
    setTimeout(() => {
      scrollToBottom()
    }, 0)
    if (!isFirstLoadScrolled.value && newValue) {
      scheduleForceScrollToBottom()
      isFirstLoadScrolled.value = true
    }
...
const handlerSubmitMsg = () => {
  const plainText = (msgContent.value || '').toString().trim()
  if (!plainText) return

  const nodeList = msgInputRef.value?.getNodeList?.() || []
  const normalizedNodeList =
    Array.isArray(nodeList) && nodeList.length > 0
      ? nodeList
      : [
          {
            type: 'text',
  ...
  let msg = {
    content: JSON.stringify(normalizedNodeList),
...
const onSendMsg = (msg) => {
  ...
  MessageApi.send({
    ...
    .then((res) => {
      if (res.code === 0) {
        const existed = res.data?.id && msgRecord.value.some((it) => it?.id === res.data.id)
        appendMessageUnique(res.data)
        if (!existed) {
          recordIndex++
        }
        scrollToBottom()
      } else {
        showToast(res.msg || '发送失败，请重试~', true)
      }
    })
    .catch((err) => {
      showToast(err?.message || '发送失败，请检查网络或服务状态~', true)
    })
    .finally(() => {
